...
# 运行时目录（支持 PyInstaller 打包：frozen 时使用 exe 所在目录）
def _runtime_dir():
...
class ScheduleManager:
    ...
    def start_web_server(self):
        ...
        try:
            ...
            # 打包后优先使用独立的 exe；源码模式用 python 直接跑 web_server.py
            if getattr(sys, "frozen", False):
                web_server_exe = os.path.join(RUNTIME_DIR, 'schedule_manager_web.exe')
                if not os.path.exists(web_server_exe):
                    logger.warning("schedule_manager_web.exe 不存在，跳过启动Web服务器")
                    return
                popen_args = [web_server_exe, '--host', '127.0.0.1', '--port', '5011']
            else:
                ...
                if not os.path.exists(web_server_path):
                    ...
                    return
                popen_args = [sys.executable, 'web_server.py', '--host', '127.0.0.1', '--port', '5011']
